...
import { Home, Building2, Car, Send, CheckCircle, ChevronRight, Phone, MapPin, Loader2 } from "lucide-react";
...
const QuoteTool = () => {
  ...
  const detectLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      (pos) => {
        ...
        setGeoLoading(false);
        toast({ title: "Location captured", description: "GPS coordinates saved with your request." });
      },
      () => setGeoLoading(false),
      { timeout: 8000 },
    );
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const res = await fetch(CMS_URL + "/api/submit-quote", {
        method: "POST",
      ...
      <section id="quote" className="py-20">
        <div className="container mx-auto px-4">
          <div className="max-w-lg mx-auto neu-card rounded-2xl p-8 text-center">
            ...
            <a href={brand.phoneNumber} className="touch-target inline-flex items-center gap-2 skeu-button text-white font-bold px-6 py-3 rounded-xl">
              <Phone className="w-5 h-5" /> Can not Wait? Call Now
            </a>
          </div>
        </div>
      </section>
    ...
    <section id="quote" className="py-20 relative">
      <div className="container mx-auto px-4">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
          <SectionHeader
            heading={sectionsSettings.quote.heading}
            subheading={sectionsSettings.quote.subheading}
            sectionKey="quote"
            richHeading={sectionsSettings.quote.richHeading}
            richSubheading={sectionsSettings.quote.richSubheading}
          />
        </motion.div>
              ...
              <motion.div key="s1" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
                ...
                <div className="space-y-3">
                  {SERVICE_OPTIONS.map(opt => (
                    <button key={opt.type} onClick={() => { setServiceType(opt.type); setStep(2); }}
                      className={"touch-target w-full flex items-center gap-4 p-4 rounded-xl transition-all text-left " + (serviceType === opt.type ? "neu-card-pressed border-accent bg-accent/5" : "neu-card hover:bg-secondary")}>
            ...
            {step === 2 && (
              <motion.div key="s2" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
                ...
                <textarea value={body} onChange={e => setBody(e.target.value)}
                  placeholder="e.g. I am locked out of my house, the key broke in the lock..."
            ...
            {step === 3 && (
              <motion.div key="s3" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
                ...
                <input type="text" value={location} onChange={e => setLocation(e.target.value)}
                  placeholder="Address or area in Cleveland" className={inputCls} />
                <button onClick={detectLocation} disabled={geoLoading}
                  className="touch-target mt-3 w-full flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl neu-card text-sm font-medium hover:bg-secondary transition-colors disabled:opacity-50">
              ...
              <motion.div key="s4" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
                ...
                <div className="flex gap-3 mt-6">
                  ...
                  <button onClick={handleSubmit} disabled={submitting}
                    className="touch-target flex-1 flex items-center justify-center gap-2 skeu-cta-red text-white font-bold px-6 py-3 rounded-xl disabled:opacity-60">
