...
export const WhatsAppConnectionPanel: React.FC = () => {
  ...
  const startSession = async () => {
    ...
    try {
      ...
      // Poll for QR refresh and connection
      qrPollRef.current = setInterval(async () => {
        const statusRes = await fetch("/api/waha?action=status")
    ...
    <div style={{ marginBottom: "1.5rem", padding: "1.25rem", borderRadius: "8px", border: "1px solid #333", background: "#1a1a2e" }}>
      ...
      {showingQr && !connected && (
        <div style={{ textAlign: "center", marginBottom: "0.75rem", padding: "1rem", background: "#111", borderRadius: "8px" }}>
          ...
          {qrData ? (
            <img src={qrData} alt="WhatsApp QR Code" style={{ width: 280, height: 280, borderRadius: "8px", border: "3px solid #25d366" }} />
          ) : (
            <div style={{ width: 280, height: 280, display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto", background: "#222", borderRadius: "8px", color: "#888", fontSize: "0.85rem" }}>
              Generating QR code...
            </div>
          )}
